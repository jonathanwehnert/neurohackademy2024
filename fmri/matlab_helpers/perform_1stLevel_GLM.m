function perform_1stLevel_GLM(BIDS_dir, filters, standard_space, native_space, names, cfg)
...
tTotal = tic();
%% user input (settings)
% BIDS_dir = '/home/jonathan/Documents/data/FL_BILINGUAL/pilot/bids_5';
% filters.subj        = '01';
% filters.ses         = '01';
...
addpath(genpath(fullfile(BIDS_dir, 'code')));
...
[fn, cfg, lss] = extractOnsets(BIDS.raw, filters, glm, cfg, fn, out_path, names);
    ...
    for crun = 1:length(lss.onsets) % loop through runs
        for ccond = 1:length(lss.onsets{crun}) % loop through conditions within run
            ...
            for ctrial = 1:length(lss.onsets{crun}{ccond})
                ...
                if contains(condition_name, '_') % remove underscores as they would mess with BIDS filenames
